...
| `npm run publish:gh-pages`  | 已提交并推送当前改动后，需要手动刷新发布时 | 本地检查后触发唯一的 `gh-pages` 工作流         |
| `npm run publish:status`    | 想确认自动发布是否结束时                   | 显示唯一发布工作流的最近运行状态               |
...
| `scripts/publish-gh-pages.mjs`                    | 发布辅助脚本；执行本地检查和静态构建，再触发唯一的 `gh-pages` 发布工作流。检测到未提交改动时会停止。 | 想用一条 npm 命令手动刷新 GitHub Pages 时。                         |
...
| `.github/workflows/deploy-gh-pages.yml`           | 唯一 GitHub Pages 工作流；检查代码、构建静态站、验证入口文件，再将产物推送到 `gh-pages` 分支。       | 更改 Pages 部署逻辑、Node 版本或分支触发条件时。                    |
...
| 步骤 | 在哪里做    | 需要做什么                                                                                                          |
| ---- | ----------- | ------------------------------------------------------------------------------------------------------------------- |
| 1    | 本地终端    | 执行 `git add .`、`git commit -m "Deploy site"` 和 `git push -u origin main`。                                      |
| 2    | GitHub 仓库 | 在 **Settings → Pages**，选择 **Deploy from a branch**，分支选择 **`gh-pages`**，目录选择 **`/(root)`**，然后保存。 |
| 3    | GitHub 仓库 | 在 **Settings → Actions → General** 确认 Workflow permissions 是 **Read and write permissions**。                   |
| 4    | GitHub 仓库 | 在 **Actions** 查看 “Deploy Sky Navigation to GitHub Pages” 成功完成；随后访问项目页。                              |

### 一键检查并触发发布

项目根目录提供 `scripts/publish-gh-pages.mjs`，将“类型检查 → 静态构建 → 触发唯一 GitHub Actions 工作流”收敛为一个 npm 命令。为避免脚本发布远程旧版本，它会在检测到未提交改动时停止；请先执行 `git add .`、`git commit -m "更新站点"` 和 `git push`。
